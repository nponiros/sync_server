...
function getServerChanges(db, syncedRevision, clientIdentity, partialsThreshold) {
  ...
  return db.getChangesData(syncedRevision, clientIdentity)
      ...
      .then(({ changes, partial }) => ({
        ...
        // If we have no new changes keep the old revision
        currentRevision: changes[changes.length - 1] ? changes[changes.length - 1].rev : syncedRevision,
      }))
