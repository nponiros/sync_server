...
function applyClientChanges(db, baseRevision, nextRevision, clientChanges, clientIdentity) {
  function handleCreate(change) {
    return db.addData(change.table, change.obj)
      .then(() => {
    ...
    return db.getData(change.table, change.key)
      .then((data) => {
        if (data) {
          ...
          return db.updateData(change.table, data)
            .then(() => {
              return db.addChangesData({
                rev: nextRevision,
                source: clientIdentity,
