...
const PartialsTable = require('./partials_table');
...
const partialsTableName = 'partials.table';
...
class DB {
  ...
  _loadTables(dataPath, tableNames, dbOptions) {
    ...
    this.partialChanges = new PartialsTable(path.join(dataPath, partialsTableName), dbOptions);
  }
  ...
  getClientRevision(clientID) {
    if (this.meta.clients[clientID]) {
      return this.meta.clients[clientID].revision;
    }
    return 0;
  }
